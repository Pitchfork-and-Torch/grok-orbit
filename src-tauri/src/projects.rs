...
const CATALOG: &[Spec] = &[
    Spec {
        ...
        name: "VELA",
        rel_paths: &["vela"],
        keywords: &["vela", "jon-11", "jon-12", "jon-13", "interval n", "compose cuts"],
    ...
    Spec {
        ...
        keywords: &["ghost continuum", "ghost-lan", "ghost #", "landing wipe"],
    ...
    Spec {
        id: "axiom",
        name: "AXIOM",
        rel_paths: &["axiom-wear"],
        keywords: &["axiom-wear", "axiom wear", "axiom"],
        remotes: &[],
    },
    Spec {
        id: "safedeposit",
        name: "SafeDeposit",
        rel_paths: &["SafeDeposit", "safedeposit"],
        keywords: &["safedeposit", "safe deposit", "safedepositbox"],
        remotes: &[],
    },
    Spec {
        id: "orbitstack",
...
fn build_catalog(home: &Path) -> Vec<Well> {
    ...
    for spec in CATALOG {
        ...
        let mut remotes: Vec<String> = spec
            .remotes
            .iter()
...
fn keywords_for(id: &str) -> &'static [&'static str] {
    CATALOG
        .iter()
        .find(|s| s.id == id)
        .map(|s| s.keywords)
