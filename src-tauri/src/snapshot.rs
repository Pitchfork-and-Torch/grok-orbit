...
pub fn collect_snapshot_ex(force_full: bool) -> Snapshot {
    ...
    let mut projects = projects::link_sessions(&mut sessions);
    crate::sample::attach(&mut sessions, &mut projects);
    let ms_merge = last.elapsed_ms();
    last = InstantLike::now();
    let mut situation = situation_text(&sessions, &projects, &adapters, &surfaces, &attention);
    if !crate::sample::has_real(&sessions) {
        situation = format!(
            "Sample well on. Connect Grok CLI, Grok Bot, or grok.com to replace this demo. {situation}"
        );
    }
    let ms_sit = last.elapsed_ms();
    last = InstantLike::now();
    let mut activity = collect_activity(&sessions);
    if !crate::sample::has_real(&sessions) {
        activity = crate::sample::activity();
    }
...
pub fn session_detail(id: &str) -> Result<SessionDetail, String> {
    if let Some(detail) = crate::sample::detail(id) {
        return Ok(detail);
    }
    if is_web_id(id) {
...
fn process_scan(live: &[Session]) -> (Surfaces, AdapterStatus) {
    ...
    // Cheap cook.json only. Receipt overlay lives on cook_status / collect.
    let cook = crate::cook::load_state();
    let surfaces = Surfaces {
        ...
        cook_armed: cook.armed,
        cook_detail: cook.last_detail.clone(),
        cook_summary: cook.last_summary.clone(),
        cook_staff: cook.staff_now,
    };
