...
export default function App() {
  const [gate, setGate] = useState<"boot" | "setup" | "app">("boot");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const st = await invoke<{ unlocked?: boolean; licensed: boolean; setup_complete: boolean }>(
          "license_status",
        );
        if (cancelled) return;
        setUnlocked(Boolean(st.unlocked ?? st.licensed));
        if (st.setup_complete) setGate("app");
        else setGate("setup");
      } catch {
        if (!cancelled) setGate("setup");
      }
    })();
    return () => {
      cancelled = true;
  ...
  function needUnlock(): boolean {
    ...
    setUnlockOpen(true);
    flashToast("Observe is free. Unlock to act.", 3200, "error");
    return true;
  }

  async function toggleCook() {
    if (!snap?.surfaces.cook_armed && needUnlock()) return;
  ...
  if (!starMode && gate === "boot") {
    return (
      <div className="setup-shell">
        <p className="setup-kicker">Orbit</p>
        <p>Starting...</p>
      </div>
    );
  }
  if (!starMode && gate === "setup") {
    return <Setup onDone={() => setGate("app")} />;
  }
  ...
  return (
    <div className={`app ${starMode ? "star-mode" : ""} ${snap?.surfaces.cook_armed ? "cook-on" : ""}`}>
      ...
      {unlockOpen && !unlocked && (
        <div className="unlock-modal" role="dialog" aria-label="Unlock Orbit actions">
          <div className="setup-card">
            <p className="setup-kicker">Unlock</p>
            <h2>Pay once to act</h2>
            <Unlock
              onDone={() => {
                setUnlocked(true);
                setUnlockOpen(false);
                flashToast("Actions unlocked");
              }}
      ...
      <header className="sit">
        <div className="sit-lead">
          {!unlocked && (
            <p className="unlock-bar" role="status">
              Observe is free. Unlock $19 to resume, COOK, ACP, and hand off.{" "}
              <button className="setup-ghost" onClick={() => setUnlockOpen(true)}>
                Unlock
              </button>
            </p>
          )}
              ...
              <div className="actions">
                {detail.session.id.startsWith("web:") ? (
                  ...
                  <button className="primary" onClick={() => {
                    if (detail.session.source === "sample") {
                      flashToast("Sample card. Connect a real Grok CLI session.", 3200, "error");
                      return;
                    }
                    if (needUnlock()) return;
                    void act("resume_session", detail.session.id, "Resume launched");
                  }}>
                    Resume in Grok
                  </button>
                )}
                ...
                {!detail.session.live && (
                  <button onClick={() => {
                    if (needUnlock()) return;
                    void continueHandoff(detail.session.id);
                  }}>Continue in Orbit ACP</button>
                )}
                {!starMode && (
