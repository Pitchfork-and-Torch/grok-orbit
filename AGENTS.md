...
Private local-first command center for Grok Build, Grok Bot, Grok web, and Cursor.
...
Proprietary. Private GitHub only (`Pitchfork-and-Torch/grok-orbit`). Do not make public. Do not tweet. No visitor counter, no i18n, no live-sites registry.
...
Phase 12 Relay: Star lists other surfaces on the same named well. `y` opens the Relay picker (clipboard, new ACP, focus+copy to a sibling live pager, Cursor follow-up, open PR/chat, desk announce). Packs include PR state/files, desk, and well members. Focus+copy never injects. New ACP uses the clone path, not a project slug. MCP adds `orbit_next`, `orbit_well`, `orbit_relay_pack` (read/pack only; no claim, no follow-up). Clearance rows have one verb. Compose with `desk.py`.
